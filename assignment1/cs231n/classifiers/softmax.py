...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
     scores = X[i].dot(W)
     shift_scores = scores - max(scores) #reduce the scores
     #loss += - shift_scores[y[i]] + np.log(sum(np.exp(shift_scores)))
     loss += -np.log(np.exp(shift_scores[y[i]]) / sum(np.exp(shift_scores)))
     for j in xrange(num_classes):
         softmax_out = np.exp(shift_scores[j])/sum(np.exp(shift_scores))
         if j == y[i]:
             dW[:,j] += (softmax_out - 1) *X[i] 
         else: 
             dW[:,j] += (softmax_out - 0) *X[i] 

...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    self.params['b1'] = np.zeros(hidden_dim) # 0 is ok
  ...
  def loss(self, X, y=None):
    ...
    scores, output_cache = affine_forward(hidden_out, W2, b2)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    loss += 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2)) # add the reg
    #we had Divided by N in softmax_loss() function
    
    dhidden_out, grads['W2'], grads['b2'] = affine_backward(dscores, output_cache)
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i, hd in enumerate(hidden_dims):
        ...
        if self.use_batchnorm:
            self.params['gamma%d'%(i+1)] = np.ones(hd) #init the gamma and beta
